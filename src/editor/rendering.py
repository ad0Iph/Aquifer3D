...
class Rendering:
    ...
    def add_mesh_to_plotter(self, key):
        """Add the mesh corresponding to `key` to the plotter, with its color and visibility."""
    ...
    def refresh_all_actors(self):
        """Redraw all actors in the plotter based on current surfaces, colors, and visibility."""
    ...
    def restore_opacities(self):
        """Restore the opacity of all actors to 1.0."""
        for key, actor in self.actors.items():
            actor.GetProperty().SetOpacity(1.0)
        self.plotter.render()

    def toggle_visibility(self, key, state):
        """Activate or deactivate the visibility of the group `key`."""
        self.visible[key] = state
        if key in self.actors:
            self.actors[key].SetVisibility(state)

    def toggle_bounds(self):
        """Show the bounds of the model with real-world units."""
        if self.show_bounds:
            self.plotter.remove_bounds_axes()
            self.plotter.remove_bounding_box()
            self.show_bounds = False
            self.update_status("Hidden bounds and axes")
        else:
            ...
            self.update_status("Bounds shown in meters (Z exaggerated) — [I] Hide bounds")
        self.plotter.render()