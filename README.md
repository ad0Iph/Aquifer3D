...
│   ├── show_model.py         # Viewer with per-property checkboxes
│   ├── export_model.py       # PLY exporter with colors
...
python show_model.py ..\model_path

# With vertical exaggeration
python show_model.py ..\model_path --z-exag 10

# Checkboxes per k value (one color per distinct value)
python show_model.py ..\model_path --split-unique

# Repair meshes with CGAL
python show_model.py ..\model_path --split-unique --repair

# Simplify 50% of the faces
python show_model.py ..\model_path --split-unique --decimate 0.5

# Interactive clipping plane
python show_model.py ..\model_path --clip

# Split into NxN blocks
python show_model.py ..\model_path --cuts 3
```
...
python export_model.py ..\model_path

# One PLY per distinct k value
python export_model.py ..\model_path --split-unique

# Repair + simplify for 3D printing
python export_model.py ..\model_path --split-unique --repair --decimate 0.5

# One PLY per geological layer
python export_model.py ..\model_path --layers
```
...
| `--no-log` | Linear instead of logarithmic scale |
| `--split-unique` | Split by distinct property values |
| `--repair` | Repair meshes (watertight) with CGAL |
| `--decimate 0.X` | Simplify mesh (0.5 = remove 50% of faces) |
| `--simplification 0.X` | (edit_model) Simplify on load |
| `--tolerance N` | (edit_model) Clearance in mm for 3D printing |
| `--cmap name` | (edit_model) matplotlib colormap |
| `--layers` | Export per geological layer |
| `--cuts N` | Subdivide into NxN blocks |
...
4. Apply assembly tolerance with `L`
